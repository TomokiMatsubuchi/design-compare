...
func main() {
	...
	compareDesignTool := mcp.NewTool("compare_design",
		...
		mcp.WithString("figma_layout_path",
			mcp.Description("Path to a JSON file containing the Figma node list metadata (alternative to figma_layout for 'layout_tree' mode; mutually exclusive with figma_layout)"),
		),
		mcp.WithString("web_layout_path",
			mcp.Description("Path to a JSON file containing the Web DOM node list layout (alternative to web_layout for 'layout_tree' mode; mutually exclusive with web_layout)"),
		),
