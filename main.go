...
func main() {
	...
	compareDesignTool := mcp.NewTool("compare_design",
		...
		mcp.WithString("figma_layout",
			mcp.Description("JSON string representing Figma node list metadata (required for 'layout_tree' mode unless figma_layout_path is given; mutually exclusive with figma_layout_path). e.g. [{\"id\":\"1\",\"name\":\"header\",\"x\":0,\"y\":0,\"w\":1000,\"h\":100,\"parent\":\"1\"}]"),
		),
		mcp.WithString("web_layout",
			mcp.Description("JSON string representing Web DOM node list layout (required for 'layout_tree' mode unless web_layout_path is given; mutually exclusive with web_layout_path). e.g. [{\"selector\":\"#header\",\"x\":0,\"y\":0,\"w\":1000,\"h\":100,\"parent\":\"#header\"}]"),
		),
		mcp.WithString("figma_layout_path",
			mcp.Description("Path to a JSON file containing the Figma node list metadata (alternative to figma_layout for 'layout_tree' mode; mutually exclusive with figma_layout). The file content is a JSON array like [{\"id\":\"1\",\"name\":\"header\",\"x\":0,\"y\":0,\"w\":1000,\"h\":100,\"parent\":\"1\"}]. Note: files are read from the server's local filesystem with the server process's privileges, so only pass paths from trusted callers"),
		),
		mcp.WithString("web_layout_path",
			mcp.Description("Path to a JSON file containing the Web DOM node list layout (alternative to web_layout for 'layout_tree' mode; mutually exclusive with web_layout). The file content is a JSON array like [{\"selector\":\"#header\",\"x\":0,\"y\":0,\"w\":1000,\"h\":100,\"parent\":\"#header\"}]. Note: files are read from the server's local filesystem with the server process's privileges, so only pass paths from trusted callers"),
		),
